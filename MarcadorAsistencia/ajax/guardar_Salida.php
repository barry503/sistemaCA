...
<?php
	...
	//var para  verificar salida
		...
		$preparando = $conexionPDO->prepare("SELECT  fecha FROM salida WHERE   idempleado='$idempleado' AND fecha='$FechadeHoy'");
		#ejecuta la consulta...
        ...
        #preparando la consulta
		$sqlverificarAsistencia = $conexionPDO->prepare("SELECT  fecha FROM asistencia WHERE   idempleado='$idempleado' AND fecha='$FechadeHoy'");
		#ejecuta la consulta...
		$sqlverificarAsistencia->execute();/*array(':idempleado' => $idempleado)*/

		$verificarAsistencia =  $sqlverificarAsistencia->fetch();#la consulta $preparando devuelve true o false

		// echo $FechadeHoy;
		// si el resultado no es igual a falso
		if ($resultado != false) {
		#ejecuta ...
			$errors[] = "El empleado ya relizo <br> su salida del dia... ";
		}else if ($verificarAsistencia == false) {
		#ejecuta ...
			$errors[] = "El empleado no marco <br>su asistencia de entrada... ";
		}else {
		...
		$sqlsalida=  $conexionPDO->prepare("INSERT INTO salida(idempleado, fecha) VALUES ('$idempleado', '$FechadeHoy')");
		$SAVEsalida = $sqlsalida->execute(); /*mysqli_query($con,$sqlsalida);*/ /*con la variable $SAVEsalida efectuo la query*/
		}
...
<?php if (isset($_POST['lectorS'])==1): ?>
<?php $lectorS = $_POST['lectorS']; ?>
	<script>
  Swal.fire({
                       /*icon:'success',*/
                       title:'<img width="120px" height="130px"  src="../files/empleado/<?php echo $scrit_foto; ?>"  alt="sin foto">',
                         html: '<h4>Hasta pronto <br> <?php echo $scrit_nombre." , ".$scrit_apellido; ?></h4><br>Salida Registrada',
                          // grow: 'fullscreen',
                       footer: 'Salida del dia de hoy <?php echo date("d-M-Y"); ?>'
                   });
</script>
<?php endif ?>
